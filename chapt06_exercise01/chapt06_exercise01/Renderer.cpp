...
bool Renderer::Initialize(float screenWidth, float screenHeight)
{
	...
	mWindow = SDL_CreateWindow("Game Programming in C++ (Chapter 6) exercise 1", 100, 100,
...
void Renderer::Draw()
{
	...
	for (auto mc : mMeshComps)
	{
		...
		{
			Shader* shader = mMeshShaders[shaderName];
		...
		else
		{
			...
			{
				SDL_Log("Failed to initialize Shader for Mesh.");
				return;
			}
			mMeshShaders.emplace(shaderName, shader);
...
Mesh* Renderer::GetMesh(const std::string & fileName)
{
	...
	std::string shaderName = m->GetShaderName();
	auto iterS = mMeshShaders.find(shaderName);
	if (iterS == mMeshShaders.end())
	{
		Shader* shader = new Shader();
		if (!shader->Load("Shaders/" + shaderName + ".vert", "Shaders/" + shaderName + ".frag"))
		{
