...
constexpr int CONTROLLERSTATE_MAXNUMS = 4;
...
struct InputState
{
	...
	MouseState Mouse;
	ControllerState Controller[CONTROLLERSTATE_MAXNUMS];
};
...
public:
	...
	bool RemoveController(int idx);
	bool AddController(int idx);
private:
	float Filter1D(int input);
	Vector2 Filter2D(int inputX, int inputY);
	InputState mState;
	SDL_GameController* mController[CONTROLLERSTATE_MAXNUMS];
};
